...
<head>
    ...
    <title>Coupe du Monde 2022 - Groupes</title>
    ...
    <div class="container mx-auto px-4 pt-24 pb-12">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @foreach ($groups as $group)
                <div class="bg-[#F5F5DC] rounded-lg border border-gray-300 p-6">
                    <h2 class="text-center text-xl font-semibold text-gray-800 ">{{ $group['name'] }}</h2>  <hr>
                    <ul class="space-y-4 mt-2">
                        @foreach ($group['teams'] as $team)
                            <li class="flex items-center space-x-3">
                                <img src="{{ $team['flag'] }}" alt="Drapeau de {{ $team['name'] }}" class="w-10 h-6 object-cover" loading="lazy">
                                <span class="text-gray-700 uppercase font-medium">{{ $team['name'] }}</span>
                            </li>
                        @endforeach
                    </ul>
                </div>
