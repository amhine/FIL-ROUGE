    ...
    <nav class="fixed w-full bg-gradient-to-r from-[#C02626] to-[#FF5733] text-white shadow-lg z-50">
        <div class="container mx-auto py-4 px-6 flex items-center justify-between">
            ...
            <div class="hidden md:flex space-x-10 text-lg">
                <a href="#hebergements" class="hover:text-gray-300 transition">Mes Hébergements</a>
        ...
        <div class="md:hidden hidden bg-[#C02626] px-4 py-4" id="mobile-menu">
            <ul class="flex flex-col space-y-4">
                <li><a href="#hebergements" class="block py-2 hover:text-gray-300">Mes Hébergements</a></li>
    ...
    <div class="container mx-auto pt-24 pb-8 px-4">
        ...
        @if($totalReservations > 0)
            ...
            <div class="bg-white p-8 rounded-2xl shadow mb-8">
                <h4 class="text-2xl font-bold mb-6 text-gray-800">Statistiques par hôtel</h4>
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-200 text-gray-700">
                                <th class="p-4">Nom de l'hôtel</th>
                                <th class="p-4">Total réservations</th>
                                <th class="p-4">Revenus totaux</th>
                                <th class="p-4">Nuits réservées</th>
                        ...
                        <tbody>
                            @foreach($statistiquesParHotel as $hotelId => $stats)
                                <tr class="border-b hover:bg-gray-100 transition">
                                    <td class="p-4">{{ $stats['nom_hotel'] }}</td>
                                    <td class="p-4">{{ $stats['total_reservations'] }}</td>
                                    <td class="p-4">{{ number_format($stats['revenu_total'], 2) }} MAD</td>
                                    <td class="p-4">{{ $stats['nuits_reservees'] }}</td>
                                    <td class="p-4">
                                        <a href="{{ route('hebergeur.hotel.detail', $hotelId) }}" class="text-blue-500 hover:underline">
                                            Voir détails
                                        </a>
                                    </td>
                                </tr>
                            ...
                            <tbody>
                                @foreach($reservations as $reservation)
                                    <tr class="border-b hover:bg-gray-100 transition">
                                        ...
                                        <td class="p-4">{{ $reservation->hotel->nom_hotel }}</td>
                                        <td class="p-4">{{ $reservation->tourist->name ?? 'Touriste inconnu' }}</td>
                                        <td class="p-4">{{ \Carbon\Carbon::parse($reservation->date_debut)->format('d/m/Y') }}</td>
                                        <td class="p-4">{{ \Carbon\Carbon::parse($reservation->date_fin)->format('d/m/Y') }}</td>
                                        <td class="p-4">{{ $reservation->nombre_nuits }}</td>
                                        <td class="p-4">{{ number_format($reservation->prix_total, 2) }} MAD</td>
                                        <td class="p-4">
                                            <span class="px-2 py-1 text-xs font-semibold rounded-full {{ 
                                                $reservation->status == 'confirmer' ? 'bg-green-100 text-green-700' : 
