...
<head>
    ...
    <title>Paiement - {{ $reservation->restaurant->nom_resteau }}</title>
    ...
    <div class="bg-white shadow-2xl rounded-lg p-8 w-96 text-center transform transition-transform hover:scale-105">
        ...
        <div class="mb-6 text-left bg-gray-50 p-4 rounded-lg">
            ...
            <p class="text-lg font-bold mt-3 text-gray-800">
                Total : <span class="text-blue-500">{{ $reservation->restaurant->prix }} DH</span>
            </p>
        </div>
        
        <form action="{{ route('paiement.process.resteau') }}" method="POST">
            @csrf
        ...
        <a href="{{ route('restaurants.search') }}" class="block text-center text-gray-600 mt-4 hover:underline">Annuler et retourner aux annonces</a>
    </div>
</body>
</html>